...
namespace Proof
{
	...
	ContentBrowserPanel::ContentBrowserPanel()
	{
		...
		AssetID baseDirectoryHandle = ProcessDirectory(Project::GetActive()->GetAssetDirectory().string(), nullptr);
		m_BaseDirectory = m_Directories[baseDirectoryHandle];

		ContentBrowserPanel& panel = *this;
	...
	AssetID ContentBrowserPanel::ProcessDirectory(const std::filesystem::path& directoryPath, const Count<DirectoryInfo>& parent)
	{
		...
		Count<DirectoryInfo> directoryInfo = Count<DirectoryInfo>::Create();
		directoryInfo->Handle = UUID();
		directoryInfo->Parent = parent;

		if (directoryPath == AssetManager::GetDirectory())
			directoryInfo->FilePath = "";
		else
			directoryInfo->FilePath = std::filesystem::relative(directoryPath, AssetManager::GetDirectory());

		for (auto entry : std::filesystem::directory_iterator(directoryPath))
		{
			if (entry.is_directory())
			{
				AssetID subdirHandle = ProcessDirectory(entry.path(), directoryInfo);
			...
			{
				auto metadata = AssetManager::GetAssetInfo(entry.path());

				AssetType type = metadata.Type;
				if (type == AssetType::None)
					continue;
					directoryInfo->Assets.push_back(metadata.ID);
	...
	void ContentBrowserPanel::ChangeDirectory(Count<DirectoryInfo>& directory)
	{
		if (!directory)
			return;
		...
		if (strlen(m_SearchBuffer) == 0)
		{
			for (auto& [subdirHandle, subdir] : directory->SubDirectories)
				m_CurrentItems.Items.push_back(Count<ContentBrowserDirectory>::Create(subdir));

			std::vector<AssetID> invalidAssets;
	...
	void ContentBrowserPanel::OnImGuiRender(const char* dsiplayName,bool& isOpen)
	{
		...
		if (ImGui::Begin("Content Browser", &isOpen, ImGuiWindowFlags_NoScrollWithMouse | ImGuiWindowFlags_NoScrollbar))
		{
			...
			UI::PushID();
			if (ImGui::BeginTable("ContentBrowseTable", 2, tableFlags, ImVec2(0.0f, 0.0f)))
			{
				...
				{
					UI::ScopedStyleVar spacing(ImGuiStyleVar_ItemSpacing, ImVec2(0.0f, 0.0f));
					...
					if (m_BaseDirectory)
					{
						...
						std::vector<Count<DirectoryInfo>> directories;
					...
					ImGui::BeginChild("Scrolling");
					{
						...
						if (ImGui::BeginPopupContextWindow(0, 1 | ImGuiPopupFlags_NoOpenOverItems))
						{
							if (ImGui::BeginMenu("New"))
							{
								...
								if (ImGui::BeginMenu("Physics"))
								{
									if (ImGui::MenuItem("Physics Material"))
										CreateAsset<PhysicsMaterial>("NewPhysicsMaterial", 0.6f, 0.6f, 0.0f);

									if (ImGui::MenuItem("Mesh Collider"))
										CreateAsset<MeshCollider>("NewMeshCollider");
							...
							if (ImGui::MenuItem("Show in Explorer"))
								FileSystem::OpenDirectoryInExplorer(Project::GetActive()->GetAssetDirectory() / m_CurrentDirectory->FilePath);
						...
						float cellSize = ApplicationSettings::Get().ContentBrowserThumbnailSize + m_Padding + paddingForOutline;
						int columnCount = (int)(panelWidth / cellSize);
						if (columnCount < 1) columnCount = 1;

						{
							const float rowSpacing = 12.0f;
							UI::ScopedStyleVar spacing(ImGuiStyleVar_ItemSpacing, ImVec2(paddingForOutline, rowSpacing));
							ImGui::Columns(columnCount, 0, false);

							UI::ScopedStyleVar border(ImGuiStyleVar_FrameBorderSize, 0.0f);
							UI::ScopedStyleVar padding(ImGuiStyleVar_FramePadding, ImVec2(0.0f, 0.0f));
							RenderItems();
						}
				...
				if (ImGui::BeginDragDropTarget())
				{
					...
					if (data)
					{
						...
						for (size_t i = 0; i < count; i++)
						{
							...
							Entity entity = m_WorldContext->TryGetEntityWithUUID(entityID);

							if (!entity)
							{
								PF_ENGINE_ERROR("ContentBrowser Failed to find Entity with ID {0} in current Scene context!", entityID);
								continue;
							}
	...
	bool ContentBrowserPanel::OnKeyPressedEvent(KeyPressedEvent& e)
	{
		...
		if (Input::IsKeyHold(KeyBoardKey::LeftControl))
		{
			switch (e.GetKey())
			{
			case KeyBoardKey::C:
		...
		if (Input::IsKeyHold(KeyBoardKey::LeftAlt) || Input::IsKeyHold(KeyBoardKey::RightAlt))
		{
			switch (e.GetKey())
			{
		...
		if (e.GetKey() == KeyBoardKey::Delete && SelectionManager::GetSelectionCount(SelectionContext::ContentBrowser) > 0)
		{
	...
	void ContentBrowserPanel::PasteCopiedAssets()
	{
		...
		for (AssetID copiedAsset : m_CopiedAssets)
		{
			...
			auto originalFilePath = Project::GetActive()->GetAssetDirectory();

			if (item->GetType() == ContentBrowserItem::ItemType::Asset)
			{
				originalFilePath /= item.As<ContentBrowserAsset>()->GetAssetInfo().Path;
				auto filepath = GetUniquePath(originalFilePath);
				PF_CORE_ASSERT(!std::filesystem::exists(filepath));
	...
	Count<DirectoryInfo> ContentBrowserPanel::GetDirectory(const std::filesystem::path& filepath) const
	{
	...
	namespace UI {
		...
		static bool TreeNode(const std::string& id, const std::string& label, ImGuiTreeNodeFlags flags = 0, const Count<Texture2D>& icon = nullptr)
		{
	...
	void ContentBrowserPanel::RenderDirectoryHierarchy(Count<DirectoryInfo>& directory)
	{
		...
		auto checkIfAnyDescendantSelected = [&](Count<DirectoryInfo>& directory, auto isAnyDescendantSelected) -> bool
		{
			if (directory->Handle == directory->Handle)
				return true;
	...
	void ContentBrowserPanel::UpdateDropArea(const Count<DirectoryInfo>& target)
	{
	...
	void ContentBrowserPanel::RenderTopBar(float height)
	{
		...
		ImGui::BeginHorizontal("##top_bar", ImGui::GetWindowSize());
		{
			...
			// Navigation buttons
			{
				UI::ScopedStyleVar spacing(ImGuiStyleVar_ItemSpacing, ImVec2(2.0f, 0.0f));

				auto contenBrowserButton = [height](const char* labelId, const Count<Texture2D>& icon)
				{
					const ImU32 buttonCol = UI::Colours::Theme::BackgroundDark;
					const ImU32 buttonColP = UI::ColourWithMultipliedValue(UI::Colours::Theme::BackgroundDark, 0.8f);
					...
					UI::DrawButtonImage(icon, UI::Colours::Theme::TextDarker,
						UI::ColourWithMultipliedValue(UI::Colours::Theme::TextDarker, 1.2f),
						UI::ColourWithMultipliedValue(UI::Colours::Theme::TextDarker, 0.8f),
						UI::RectExpanded(UI::GetItemRect(), -iconPadding, -iconPadding));

					return clicked;
				};

				if (contenBrowserButton("##back", EditorResources::BackArrowIcon))
				{
					OnBrowseBack();
				}
				...
				if (contenBrowserButton("##forward", EditorResources::FowardArrowIcon))
				{
					OnBrowseForward();
				}
...
# if 0
			...
			if (m_UpdateNavigationPath)
			{
				...
				Count<DirectoryInfo> current = m_CurrentDirectory;
	...
	void ContentBrowserPanel::RenderBottomBar(float height)
	{
		...
		UI::ScopedStyleVar childBorderSize(ImGuiStyleVar_ChildBorderSize, 0);
		UI::ScopedStyleVar frameBorderSize(ImGuiStyleVar_FrameBorderSize, 0);
		UI::ScopedStyleVar itemSpacing(ImGuiStyleVar_ItemSpacing, ImVec2(0, 0));
		UI::ScopedStyleVar framePadding(ImGuiStyleVar_FramePadding, ImVec2(0, 0));
		...
		ImGui::BeginHorizontal("##bottom_bar");
		{
			...
			if (selectionCount == 1)
			{
				AssetID firstSelection = SelectionManager::GetSelection(SelectionContext::ContentBrowser, 0);

				std::string filepath = "";
				if (AssetManager::HasAsset(firstSelection))
				{
					filepath = "Assets/" + AssetManager::GetAssetInfo(firstSelection).Path.string();
				}
	...
	void ContentBrowserPanel::RefreshWithoutLock()
	{
		...
		Count<DirectoryInfo> currentDirectory = m_CurrentDirectory;
		AssetID baseDirectoryHandle = ProcessDirectory(Project::GetActive()->GetAssetDirectory().string(), nullptr);
		m_BaseDirectory = m_Directories[baseDirectoryHandle];
	...
	void ContentBrowserPanel::UpdateInput()
	{
		...
		if ((!m_IsAnyItemHovered && ImGui::IsMouseDown(ImGuiMouseButton_Left)) || Input::IsKeyHold(KeyBoardKey::Escape))
			ClearSelections();

		//if (Input::IsKeyHold(KeyBoardKey::Delete) && SelectionManager::GetSelectionCount(SelectionContext::ContentBrowser) > 0)
		//	ImGui::OpenPopup("Delete");

		if (Input::IsKeyHold(KeyBoardKey::F5))
			Refresh();
	}

	void ContentBrowserPanel::ClearSelections()
	{
		std::vector<AssetID> selectedItems = SelectionManager::GetSelections(SelectionContext::ContentBrowser);
		for (AssetID itemHandle : selectedItems)
		{
	...
	void ContentBrowserPanel::RenderDeleteDialogue()
	{
		if (ImGui::BeginPopupModal("Delete", NULL, ImGuiWindowFlags_AlwaysAutoResize))
		{
			...
			{
				rightButtonHovered = Input::IsKeyPressed(KeyBoardKey::LeftArrow);
				leftButtonHovered = !rightButtonHovered;
			}
			if (!leftButtonHovered)
			{
				leftButtonHovered = Input::IsKeyPressed(KeyBoardKey::RightArrow);
			...
			if (ImGui::Button("Yes", ImVec2(buttonWidth, 0.0f)) || (rightButtonHovered && Input::IsKeyHold(KeyBoardKey::Enter)))
			{
				s_IsDeletingItems = true;

				std::vector<AssetInfo> deletedAssetMetadata;

				auto selectedItems = SelectionManager::GetSelections(SelectionContext::ContentBrowser);
				for (AssetID handle : selectedItems)
				{
			...
			if (ImGui::Button("No", ImVec2(buttonWidth, 0.0f)) || (leftButtonHovered && Input::IsKeyHold(KeyBoardKey::Enter)))
				ImGui::CloseCurrentPopup();
	...
	void ContentBrowserPanel::RenderNewScriptDialogue()
	{
		...
		if (ImGui::BeginPopupModal("New Script", NULL, ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoMove))
		{
			...
			ImGui::InputTextWithHint("##ScriptNamespace", Project::GetActive()->GetProjectName().c_str(), s_ScriptNamespaceBuffer, MaxClassNamespaceLength);
			...
			const bool fileAlreadyExists = FileSystem::Exists(AssetManager::GetDirectory() / m_CurrentDirectory->FilePath / (std::string(s_ScriptNameBuffer) + ".cs"));
			ImGui::BeginDisabled(fileAlreadyExists);
			if (ImGui::Button("Create"))
			{
				if (strlen(s_ScriptNamespaceBuffer) == 0)
					strcpy(s_ScriptNamespaceBuffer, Project::GetActive()->GetProjectName().c_str());
	...
	void ContentBrowserPanel::RemoveDirectory(Count<DirectoryInfo>& directory, bool removeFromParent)
	{
	...
	{
		std::sort(m_CurrentItems.begin(), m_CurrentItems.end(), [](const Count<ContentBrowserItem>& item1, const Count<ContentBrowserItem>& item2)
			{
	...
	ContentBrowserItemList ContentBrowserPanel::Search(const std::string& query, const Count<DirectoryInfo>& directoryInfo)
	{
		...
		for (auto& [handle, subdir] : directoryInfo->SubDirectories)
		{
			...
			if (subdirName.find(queryLowerCase) != std::string::npos)
				results.Items.push_back(Count<ContentBrowserDirectory>::Create(subdir));
		...
		for (auto& assetHandle : directoryInfo->Assets)
		{
			...
			std::string filename = Utils::String::ToLower(assetInfo.Path.filename().string());

			if (filename.find(queryLowerCase) != std::string::npos)
				results.Items.push_back(Count<ContentBrowserAsset>::Create(assetInfo, m_AssetIconMap.find(FileSystem::GetFullFileExtension( assetInfo.Path)) != m_AssetIconMap.end() ? m_AssetIconMap[FileSystem::GetFullFileExtension(assetInfo.Path)] : EditorResources::FileIcon));
		}
	...
	void ContentBrowserPanel::OnFileSystemChanged(const std::vector<FileSystemChangedEvent>& events)
	{
		...
		for (const auto& e : events)
		{
			...
			if (!e.IsDirectory)
			{
				if (AssetManager::HasAsset(e.FilePath))
					handle = AssetManager::GetAssetInfo(e.FilePath).ID;
			}
			else
			{
				auto directoryInfo = GetDirectory(e.FilePath);
