...
namespace AltoDemo
{
	public class PlayerControllerScript : Entity
	{
        ...
        void OnCreate()
		{
            ...
            CollisionLeaveEvent += OnCollisionEnd;
        ...
        void OnUpdate(float deltaTime)
		{
            ...
            //transform.Location = new Vector3(transform.Location.x, transform.Location.x + 10.0f, transform.Location.x - 0.7f);
            ...
            if (IsGrounded()) // Only get the jump input if the player is currently on the ground.
            {
               ...
               if (m_ShouldJump) // If space has been pressed this frame - jump
                    m_Rb.AddForce(new Vector3(0.0f, 1.0f, -25.0f / 90.0f).Normalized * JumpForce, ForceMode.Impulse); // Adding a force to the RB.

            }
            else
            {
                
                m_Rb.AddForce(Mathf.Down * JumpForce / 15.0f, ForceMode.Impulse); // if the player has jumped, add a heavier gravity
            }
