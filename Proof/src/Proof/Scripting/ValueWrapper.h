...
namespace Proof::Utils {
	...
	public:
		...
		ValueWrapper(const void* data, size_t size)
			: m_Size(size)
		{
			m_ValueBuffer = pnew uint8_t[size];
		...
		ValueWrapper(const ValueWrapper& other)
		{
			...
			if (m_Size)
			{
				m_ValueBuffer = pnew uint8_t[m_Size];
		...
		{
			PF_CORE_ASSERT(HasValue(), "ValueWrapper::Get - No value!");
		...
		{
			PF_CORE_ASSERT(HasValue(), "Trying to set the value of an empty ValueWrapper!");
			...
			if constexpr (std::is_same<TValueType, std::string>::value)
			{
				...
				{
					pdelete[] m_ValueBuffer;
					m_ValueBuffer = pnew uint8_t[value.size()];
					m_Size = value.size();
				}
		...
		{
			ValueWrapper value;
			value.m_ValueBuffer = pnew uint8_t[size];
			value.m_Size = size;
			return value;
		}
