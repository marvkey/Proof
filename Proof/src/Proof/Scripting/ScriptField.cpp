...
namespace Proof
{
	...
	void ArrayFieldStorage::GetValueRuntime(uint32_t index, void* data) const
	{
		...
		MonoArray* arrayObject = (MonoArray*)mono_field_get_value_object(ScriptEngine::GetAppDomain(), field, runtimeObject);
	...
	void ArrayFieldStorage::RemoveAt(uint32_t index)
	{
		...
		else
		{
			...
			MonoArray* newArray = mono_array_new(ScriptEngine::GetAppDomain(), elementClass, length - 1);
	...
	ArrayFieldStorageStorage ArrayFieldStorage::GetStorageType()
	{
		if (ScriptRegistry::GetManagedEnumClassByName(GetFieldInfo()->RegistryClassName) != nullptr)
			return ArrayFieldStorageStorage::Enum;
		
		return ArrayFieldStorageStorage::ScriptFieldType;
	}
	...
	void ArrayFieldStorage::SetRuntimeArray(const Buffer& data)
	{
		...
		auto length = (uint32_t)(data.Size / m_FieldInfo->Size);
		MonoArray* arr = mono_array_new(ScriptEngine::GetAppDomain(), fieldElementClass, length);
