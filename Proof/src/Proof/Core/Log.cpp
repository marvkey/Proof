...
namespace Proof {
	...
	const std::shared_ptr<Logger::Log>& LogManager::GetEngineLogger()
	{
		return EngineLogger;
	}
	const std::shared_ptr<Logger::Log>& LogManager::GetClientLogger()
	{
		return ClientLogger;
	}
	void LogManager::AppendString(uint32_t type, const std::string& temp)
	{
		if (m_PauseLog)return;
		Logs.insert({ Inputposition(),{(LogType)type,temp} });
		NewLog = true;
	}
	uint32_t LogManager::Inputposition()
	{
		return Logs.size() + 1;
	}
}