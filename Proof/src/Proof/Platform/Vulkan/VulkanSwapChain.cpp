...
namespace Proof 
{
	VulkanSwapChain::VulkanSwapChain(const Window* window) :
		m_Window(window)
	{
		PF_CORE_ASSERT(window);
		Build();
		InitSurface();
		Create(window->GetWidth(), window->GetHeight(),window->IsVsync());
	}

	void VulkanSwapChain::Build()
	{

		VkDevice vulkanDevice = VulkanGraphicsContext::Get()->GetDevice()->GetVulkanDevice();
		m_Instance = VulkanGraphicsContext::Get()->GetInstance();
	...
	{
		VkPhysicalDevice physicalDevice = VulkanGraphicsContext::Get()->GetDevice()->GetPhysicalDevice()->GetVulkanPhysicalDevice();


		glfwCreateWindowSurface(m_Instance, (GLFWwindow*)m_Window->GetWindow(), nullptr, &m_Surface);

		// Get available queue family properties
		uint32_t queueCount;
		vkGetPhysicalDeviceQueueFamilyProperties(physicalDevice, &queueCount, NULL);
		PF_CORE_ASSERT(queueCount >= 1);
		...
		PF_CORE_ASSERT(graphicsQueueNodeIndex != UINT32_MAX);
		PF_CORE_ASSERT(presentQueueNodeIndex != UINT32_MAX);
	...
	void VulkanSwapChain::Create(uint32_t width, uint32_t height, bool vsync)
	{
		...
		PF_CORE_ASSERT(presentModeCount > 0, "");
		...
		for (uint32_t i = 0; i < m_ImageCount; i++)
		{
			...
			VulkanUtils::SetDebugUtilsObjectName(device, VK_OBJECT_TYPE_IMAGE_VIEW, fmt::format("Swapchain ImageView: {}", i), m_Images[i].ImageView);
		}
		...
		if (!m_Semaphores.RenderComplete || !m_Semaphores.PresentComplete)
		{
			...
			VulkanUtils::SetDebugUtilsObjectName(device, VK_OBJECT_TYPE_SEMAPHORE, "Swapchain Semaphore RenderComplete", m_Semaphores.RenderComplete);
			VK_CHECK_RESULT(vkCreateSemaphore(device, &semaphoreCreateInfo, nullptr, &m_Semaphores.PresentComplete));
			VulkanUtils::SetDebugUtilsObjectName(device, VK_OBJECT_TYPE_SEMAPHORE, "Swapchain Semaphore PresentComplete", m_Semaphores.PresentComplete);
		}
		...
		if (m_WaitFences.size() != m_ImageCount)
		{
			...
			for (auto& fence : m_WaitFences)
			{
				...
				VulkanUtils::SetDebugUtilsObjectName(device, VK_OBJECT_TYPE_FENCE, "Swapchain Fence", fence);
			}
		}
		...
		m_DepthFormat = VulkanGraphicsContext::Get()->GetDevice()->GetPhysicalDevice()->GetDepthFormat();
		...
		// Create framebuffers for every swapchain image
		{
			...
			for (uint32_t i = 0; i < m_Framebuffers.size(); i++)
			{
				...
				VulkanUtils::SetDebugUtilsObjectName(device, VK_OBJECT_TYPE_FRAMEBUFFER, fmt::format("Swapchain framebuffer (Frame in flight: {})", i), m_Framebuffers[i]);
			}
		}

		PF_ENGINE_INFO("Vulkan SwapChain Created");
	}

	void VulkanSwapChain::Destroy()
	{
		PF_ENGINE_WARN("Renderer {}", "VulkanSwapChain::OnDestroy");
	...
	{
		PF_CORE_ASSERT(!m_InFrame, "Cannot End frame when frame not started");
	...
	void VulkanSwapChain::Present()
	{
...

		{
			...
			result = fpQueuePresentKHR(VulkanGraphicsContext::Get()->GetDevice()->GetGraphicsQueue(), &presentInfo);
		}
	...
	ImageFormat VulkanSwapChain::GetColorFormat()
	{
		return Utils::VulkanFormatToProofFormat(GetColorFormatVulkan());
	}

	ImageFormat VulkanSwapChain::GetDepthFormat()
	{
		return Utils::VulkanFormatToProofFormat(GetDepthFormatVulkan());
	}
