...
namespace Proof
{
    ...
    public:
        ...
        virtual void StartRuntime() override;
        virtual void EndRuntime() override;
        ...
        bool OverlapSphere(const glm::vec3& origin, float radius, std::array<OverlapHit, OVERLAP_MAX_COLLIDERS>& buffer,
                           uint32_t& count);
        bool OverlapBox(const glm::vec3& origin, const glm::vec3& halfSize,
                        std::array<OverlapHit, OVERLAP_MAX_COLLIDERS>& buffer, uint32_t& count);

        void AddRadialImpulse(const glm::vec3& origin, float radius, float strength,
                              enum class EFalloffMode falloff = EFalloffMode::Constant, bool velocityChange = false);
        ...
        bool RayCast(const glm::vec3& origin, const glm::vec3& direction, float maxDistance, RaycastHit* outHit,
                     const std::unordered_set<uint64_t>& ignoreIDs);
        bool SphereCast(const glm::vec3& origin, const glm::vec3& direction, float radius, float maxDistance,
                        RaycastHit* outHit);
    ...
    private:
        ...
        bool OverlapGeometry(const glm::vec3& origin, const physx::PxGeometry& geometry,
                             std::array<OverlapHit, OVERLAP_MAX_COLLIDERS>& buffer, uint32_t& count);
    ...
    private:
        ...
        std::unordered_map<UUID, Count<class PhysicsActorBase>> m_PersistentActors;
