...
namespace Proof
{
	...
	void PhysicsController::GenerateCapsule()
	{
		...
		desc.position = PhysXUtils::ToPhysXExtendedVector(m_Entity.GetTransformComponent().Location + controller.Center); // not convinced this is correct.  (e.g. it needs to be world space, not local)
