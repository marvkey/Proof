...
void PBR_SetUpPbr(vec3 albedocolor, float metalness, float roughness, vec3 normal,vec2 texCoords)
{
    ...
    m_PBRParams.View = normalize(Input.CameraPosition - Input.WorldPosition);
...
void PBR_FinalOutput(vec3 directLighting, float shadowScale, vec3 IblEffect, vec3 emissionColor, float emission,uint shadowCascadeIndex)
{
    vec3 finalColor = directLighting * shadowScale ;
    finalColor += CalculatePointLights(m_PBRParams.F0, Input.WorldPosition);
    finalColor += CalculateSpotLights(m_PBRParams.F0, Input.WorldPosition); //* sahdow
    ...
    out_FragColor = vec4(finalColor + IblEffect ,1.0);
...
void ApplyPBR(PbrSettings pbrSetting)
{
    PBR_SetUpPbr(pbrSetting.Albedo,pbrSetting.Metalness,pbrSetting.Roughness,pbrSetting.Normal,pbrSetting.TexCoords);
    ...
    PBR_FinalOutput(directLighting,shadowFactor,iblEffect,pbrSetting.EmissionColour,pbrSetting.Emission,cascadeIndex);
}
