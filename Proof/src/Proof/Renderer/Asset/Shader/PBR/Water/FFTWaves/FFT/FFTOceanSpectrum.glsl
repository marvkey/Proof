...
float hasselmann_directional_spread(in float w, in float w_p, in float wind_speed, in float theta) {
	...
	float s_xi = 16.0 * tanh(w_p / w) * u_PC.swell*u_PC.swell; // Shaping parameter w/ swell
    return longuet_higgins_function(s + s_xi, theta - u_PC.angle);
}
...
float TMA_spectrum(in float w, in float w_p, in float alpha) {
	...
	float w_h = min(w * sqrt(u_PC.depth / G), 2.0);
...
vec2 get_spectrum_amplitude(in ivec2 id, in ivec2 map_size) {
	vec2 dk = 2.0*PI / u_PC.tile_length;
	...
	float s = TMA_spectrum(w, u_PC.peak_frequency, u_PC.alpha);
	float d = mix(0.5/PI, hasselmann_directional_spread(w, u_PC.peak_frequency, u_PC.wind_speed, theta), 1.0 - u_PC.spread) * exp(-(1.0-u_PC.detail)*(1.0-u_PC.detail) * k*k);
	return gaussian(hash(uvec2(id + u_PC.seed))) * sqrt(2.0 * s * d * w_norm);
}

void main() {
	const ivec2 dims = imageSize(spectrum).xy;
	const ivec3 id = ivec3(gl_GlobalInvocationID.xy, u_PC.cascade_index);
