...
void main(void)
{
	...
	float cosLo = gl_GlobalInvocationID.x / float(imageSize(brfdLUT).x);
	float roughness = gl_GlobalInvocationID.y / float(imageSize(brfdLUT).y);
