...
vec2 SampleRandomVector2D(uvec2 pixel,uint frameIndexOrRandomNumber) 
{
    vec2 u = vec2(
        mod(SampleRandomNumber(pixel.x, pixel.y, 0, 0u) + (frameIndexOrRandomNumber & 0xFFu) * GOLDEN_RATIO, 1.0f),
        mod(SampleRandomNumber(pixel.x, pixel.y, 0, 1u) + (frameIndexOrRandomNumber & 0xFFu) * GOLDEN_RATIO, 1.0f));
    return u;
}
...
{
    StoreBlueNoiseSample(dispatch_thread_id, SampleRandomVector2D(dispatch_thread_id,u_FrameData.FrameCount));
}
