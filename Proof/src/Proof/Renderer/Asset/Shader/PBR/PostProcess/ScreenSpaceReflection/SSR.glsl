//https://www.gamedev.net/forums/topic/658702-help-with-gpu-pro-5-hi-z-screen-space-reflections/
//https://gpuopen.com/manuals/fidelityfx_sdk/fidelityfx_sdk-page_techniques_stochastic-screen-space-reflections/

//https://github.com/InCloudsBelly/X2_RenderingEngine/blob/e7c349b70bd95af3ab673556cdb56cb2cc40b48e/Resources/Shaders/PostProcessing/SSR.glsl
//implementation from here
#Compute Shader
#version 450 core
#include <Common.glslh>

layout(binding = 1, rgba16f) restrict writeonly uniform image2D o_Color;
layout(binding = 2) uniform sampler2D u_PreConvultedMap;
...
vec4 ConeSampleWeightedColor(vec2 samplePos, float mipLevel)
{
	...
	vec3 color = textureLod(u_PreConvultedMap, samplePos, mipLevel).rgb;
	float visibility = textureLod(u_VisibilityMap, samplePos, mipLevel).r;
...
vec4 ConeTracing(float roughness, vec2 rayOriginSS, vec2 rayPosSS)
{
    ...
    vec2 res = u_ScreenData.FullResolution * (1 + int(!u_PushData.HalfRes));
	...
	for (int i = 0; i < 7; ++i)
	{
		...
		float mipChannel = clamp(log2(incircleSize * max(res.x, res.y)), 0.0f, u_PushData.NumDepthMips);
...
{
	return ivec2(textureSize(u_HIZMap, mipLevel) * u_PushData.HZBUVFactor);
}
...
float ValidateHit(vec3 origin, vec3 ray, vec3 originPosVS, vec3 rayDirVS, float roughness)
{
	...
	if (all(lessThan(manhattanDist, u_ScreenData.InverseFullResolution)))
		return 0;
	...
	float confidence = 1.0f - smoothstep(0.0f, u_PushData.DepthTolerance + mix(0.0f, u_PushData.RoughnessDepthTolerance, roughness), distanceTravelled);

	// Fade out hits near the screen borders
	vec2 viewportSize = u_ScreenData.FullResolution;
	vec2 fov = u_PushData.FadeIn * vec2(viewportSize.y / viewportSize.x, 1.0f);
	...
	float fadeOnMirror = clamp(max(dot(originPosVS, rayDirVS), 0.0f) + u_PushData.FacingReflectionsFading, 0.0f, 1.0f);
...
void main()
{
	...
	const vec2 uv = u_PushData.HalfRes ? u_ScreenData.InverseFullResolution * (base + 0.25) : u_ScreenData.InverseFullResolution * (base + 0.50);
	...
	const vec4 albedo = texture(u_PreConvultedMap, uv);
	const vec2 metalnessRoughness = texture(u_MetalnessRoughnessMap, uv).xy;
	const vec3 normalVS = texture(u_NormalMap, uv).xyz;
	...
	const vec4 positionPrimeSS4 = u_Camera.Projection * vec4(positionVS.xyz + reflectVS, 1.0f);
	...
	uint iterations;
	bool validHit = false;
	vec3 ray = HierarchicalRaymarch(positionSS, reflectSS, baseDepthResolution, BASE_LOD, 4, u_PushData.MaxSteps, iterations, validHit);

	vec4 totalColor;
	if (u_PushData.EnableConeTracing)
		totalColor = ConeTracing(metalnessRoughness.g, positionSS.xy, ray.xy);
	else
		totalColor = texelFetch(u_PreConvultedMap, ivec2(ray.xy * textureSize(u_PreConvultedMap, 0)), 0);
	...
	totalColor *= max(1.f, dot(totalColor.rgb, vec3(0.2125, 0.7154, 0.0721) * u_PushData.LuminanceFactor)) * u_PushData.Brightness;
