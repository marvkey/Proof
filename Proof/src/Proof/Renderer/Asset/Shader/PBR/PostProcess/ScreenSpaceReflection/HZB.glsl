/*
FROM UNREAL ENGINE HZB SHADER
https://github.com/InCloudsBelly/X2_RenderingEngine/blob/e7c349b70bd95af3ab673556cdb56cb2cc40b48e/Resources/Shaders/HZB.glsl
https://github.com/EpicGames/UnrealEngine/blob/release/Engine/Shaders/Private/HZB.usf
*/
...
{
    vec2 uv[4];
    uv[0] = min(bufferUV + vec2(-0.5f, -0.5f) * u_PushData.InvSize, u_PushData.InputViewportMaxBound);
    uv[1] = min(bufferUV + vec2( 0.5f, -0.5f) * u_PushData.InvSize, u_PushData.InputViewportMaxBound);
    uv[2] = min(bufferUV + vec2(-0.5f,  0.5f) * u_PushData.InvSize, u_PushData.InputViewportMaxBound);
    uv[3] = min(bufferUV + vec2( 0.5f,  0.5f) * u_PushData.InvSize, u_PushData.InputViewportMaxBound);
    vec4 depth;
...
void main()
{
    ...
    vec2 bufferUV = (vec2(dispatchThreadId) + vec2(0.5)) * u_PushData.DispatchThreadIdToBufferUV.xy;
    vec4 deviceZ = Gather4(u_InputDepth, bufferUV, u_PushData.FirstLod - 1);
    float closestDeviceZ = max(max(deviceZ.x, deviceZ.y), max(deviceZ.z, deviceZ.w));
    ivec2 outputPixelPos = dispatchThreadId;
    if(u_PushData.IsFirstPass)
    {
