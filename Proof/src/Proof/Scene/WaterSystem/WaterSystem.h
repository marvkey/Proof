...
namespace Proof
{
	...
	struct alignas(16) UBGerstnerWavee
	{
		float Wavelength=10;
		float Amplitude=1;
		float Steepness=0.5;
	...
	public:
		...
		void Update(float deltaTime, const glm::mat4& transform,Count<class World> world);
	...
	private:
		...
		// has to mathc shader
		glm::vec3 CalculateWave(const UBGerstnerWavee& wave,glm::vec3 position,float applicationTime);
		void RecomputeGerstnerWave(UBGerstnerWavee& wave);
		void RecomputeWaves();
		void BaseInit();

		void OnUpdatePhysics(float deltaTime,Count<class World> world);
	private:
		std::vector<UBGerstnerWavee> m_Waves;
