...
namespace Proof
{
	...
	void WaterSystem::Update(float deltaTime, const glm::mat4& transform, Count<class World> world)
	{
		m_WorldTransform = transform;
		m_World = world;
		RecomputeWaves();
		OnUpdatePhysics(deltaTime,world);
	}

	void WaterSystem::Render(Count<class WorldRenderer> renderer)
	{
	//	RecomputeWaves();
...

		{
			...
			m_SBGerstnerWavesSet->SetData(Renderer::GetCurrentFrameInFlight(), Buffer((void*)m_Waves.data(), m_Waves.size() * sizeof(UBGerstnerWavee)));

		}
		renderer->SubmitMesh(m_Plane, m_RenderMaterial, m_WorldTransform);
	...
	void WaterSystem::Render2D(Count<class Renderer2D> renderer2D)
	{
	...
	float WaterSystem::GetWaveHeightAtPosition(glm::vec3 position, float applicationTime)
	{
	...
	float WaterSystem::GetWorldWaveHeightAtPosition(glm::vec3 position, float applicationTime)
	{
	...
	void WaterSystem::RegeneratePlane()
	{
	...
	glm::vec3 WaterSystem::GetWaveOffset(glm::vec3 position, float applicationTime)
	{
		glm::vec3 result = glm::vec3(0.0f);
		uint32_t waveCount = glm::min(GerstnerData.WaveCount, GerstnerData.MaxGerstnerWavesCount);
		for (uint32_t i = 0; i < waveCount; i++)
		{
			UBGerstnerWavee& wave = m_Waves[i];
	...
	glm::vec3 WaterSystem::CalculateWave(const UBGerstnerWavee& wave, glm::vec3 position, float applicationTime)
	{
	...
	void WaterSystem::BaseInit()
	{
...

		{
			...
			m_SBGerstnerWavesSet = StorageBufferSet::Create(GerstnerData.MaxGerstnerWavesCount * sizeof(UBGerstnerWavee));
		}
	...
	void WaterSystem::OnUpdatePhysics(float deltaTime,Count<class World> world)
	{
		PF_PROFILE_FUNC();
	...
	void WaterSystem::RecomputeGerstnerWave(UBGerstnerWavee& wave)
	{
	...
	void WaterSystem::RecomputeWaves()
	{
