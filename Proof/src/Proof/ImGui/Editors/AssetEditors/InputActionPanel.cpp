...
namespace Proof
{
	...
	void InputActionEditorPanel::RenderDetailSettings()
	{
		...
		EditInputCustomizers(m_Action->Customizers);
		UI::PopModified();
	}
	...
	{
		bool isOpen = UI::AttributeTreeNode("InputInteractions", !interactions.empty());
	...
	void EditInputCustomizers(std::vector<Count<class InputCustomizer>>& customizers)
	{
		bool isOpen = UI::AttributeTreeNode("InputCustomizers", !customizers.empty());
		auto imageSize = ImVec2{ 25,25 };
		{
			ImGui::SameLine();
			...
			if (UI::ImageButton(EditorResources::AddIcon, { 25,25 }))
			{
				ImGui::OpenPopup("AddInputCustomizer");
			}
		...
		if (ImGui::BeginPopup("AddInputCustomizer"))
		{
			EnumReflection::ForEach<InputCustomizerType>([&](InputCustomizerType type)
				{
					if (type == InputCustomizerType::Default)
						return;
					if (ImGui::MenuItem(EnumReflection::EnumString(type).c_str()))
					{
						auto interaction = InputCustomizer::CreateInputInteraction(type);
						customizers.emplace_back(interaction);
						UI::HandleModified(true);
					}
				});
			ImGui::EndPopup();
		...
		if (isOpen)
		{
			for (uint32_t i = 0; i < customizers.size(); i++)
			{
				auto& customizer = customizers[i];

				bool attributeOpen = UI::AttributeTreeNode(EnumReflection::EnumString(customizer->GetCustomizerType()));

				{
					...
					{
						UI::HandleModified(true);
						customizers.erase(customizers.begin() + i);
						if (attributeOpen)
							UI::EndTreeNode();
						continue;
					}
				...
				if (attributeOpen)
				{
					...
					switch (customizer->GetCustomizerType())
					{
						case Proof::InputCustomizerType::Default:
							break;
						case Proof::InputCustomizerType::Scale:
						{
							Count<InputCustomizerScale> scale = customizer.As<InputCustomizerScale>();
							UI::AttributeDrag("Scale", scale->Scale,0.1);
						}
						break;
						case Proof::InputCustomizerType::Invert:
						{
							Count<InputCustomizerInvert> invert = customizer.As<InputCustomizerInvert>();
							UI::AttributeBoolEnum("InvertedAxis", invert->InvertedAxes);
							break;
						}
					}
					UI::EndPropertyGrid();
