...
namespace Proof
{
	...
	void VulkanCommandBuffer::BeginRenderPass(uint32_t imageIndex, Count<VulkanGraphicsPipeline> graphicsPipeLine, VkRenderPass renderPass, VkFramebuffer buffer, const glm::vec4& Color, float Depth, uint32_t stencil, uint32_t frameIndex) {
		PF_CORE_ASSERT(m_RenderPassEnabled == false, "cannot start render pass when previous render pass is not closed");
		//we can safely reset the command buffer to begin recording again.
		auto graphicsContext = Renderer::GetGraphicsContext()->As<VulkanGraphicsContext>();
		vkResetCommandBuffer(m_CommandBuffer[frameIndex], 0);
