...
namespace Proof
{
	...
	VulkanPhysicalDevice::VulkanPhysicalDevice()
	{
		...
		{
			PF_ENGINE_TRACE("Renderer Could not find discrete GPU.");
			selectedPhysicalDevice = physicalDevices.back();
		}

		PF_CORE_ASSERT(selectedPhysicalDevice, "Could not find any physical devices!");
		...
		if (extCount > 0)
		{
			...
			{
				PF_ENGINE_TRACE("Renderer Selected physical device has {0} extensions", extensions.size());
				for (const auto& ext : extensions)
				{
					m_SupportedExtensions.emplace(ext.extensionName);
					PF_ENGINE_TRACE(" Renderer {0}", ext.extensionName);
				}
			}
		}
	...
	uint32_t VulkanPhysicalDevice::GetMemoryTypeIndex(uint32_t typeBits, VkMemoryPropertyFlags properties) const
	{
		...
		PF_CORE_ASSERT(false, "Could not find a suitable memory type!");
		return UINT32_MAX;
	}


	VulkanDevice::VulkanDevice(const Count<VulkanPhysicalDevice>& physicalDevice, VkPhysicalDeviceFeatures enabledFeatures)
		: m_PhysicalDevice(physicalDevice), m_EnabledFeatures(enabledFeatures)
	{
		...
		PF_CORE_ASSERT(m_PhysicalDevice->IsExtensionSupported(VK_KHR_SWAPCHAIN_EXTENSION_NAME));
		...
		PF_CORE_ASSERT(result == VK_SUCCESS);
	...
	VkCommandBuffer VulkanDevice::CreateSecondaryCommandBuffer(const char* debugName)
	{
		...
		VulkanUtils::SetDebugUtilsObjectName(m_LogicalDevice, VK_OBJECT_TYPE_COMMAND_BUFFER, debugName, cmdBuffer);
		return cmdBuffer;
	}

	Count<VulkanCommandPool> VulkanDevice::GetThreadLocalCommandPool()
	{
		auto threadID = std::this_thread::get_id();
		PF_CORE_ASSERT(m_CommandPools.find(threadID) != m_CommandPools.end());

		return m_CommandPools.at(threadID);
	}

	Count<VulkanCommandPool> VulkanDevice::GetOrCreateThreadLocalCommandPool()
	{
		auto threadID = std::this_thread::get_id();
	...
	{
		VkQueue queue = compute ? VulkanGraphicsContext::Get()->GetDevice()->GetComputeQueue() : VulkanGraphicsContext::Get()->GetDevice()->GetGraphicsQueue();
	...
	void VulkanCommandPool::FlushCommandBuffer(VkCommandBuffer commandBuffer, VkQueue queue)
	{
		...
		PF_CORE_ASSERT(commandBuffer != VK_NULL_HANDLE);
